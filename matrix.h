...
public:
	...
	virtual ~Matrix(); //virtual, damit abgeleitete Klassen sich mit eigenem Konstruktor löschen lassen
	...
	Matrix<T>& operator*=(const Matrix<T>& B);//Matrix*Matrix
	Matrix<T> Matrix<T>::classic_multiplication(const Matrix<T>& B);
	...
	void Matrix<T>::transpose(void); //Transpose
	Matrix<T> Matrix<T>::transposeR(void); //Transpose mit Rückgabewert
